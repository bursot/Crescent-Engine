# Third-Party Notices

This project uses third-party software. License texts are included where noted.

## JoltPhysics
- Source: https://github.com/jrouwe/JoltPhysics
- License: MIT
- Fetched by `scripts/setup_deps.sh`

## assimp
- Source: https://github.com/assimp/assimp
- License: BSD 3-Clause
- Fetched by `scripts/setup_deps.sh`

## metal-cpp
- Source: https://github.com/apple/metal-cpp
- License: MIT (see `metal-cpp/LICENSE.txt`)

## nlohmann/json
- Source: https://github.com/nlohmann/json
- License: MIT (license header in `ThirdParty/nlohmann/json.hpp`)

## stb_image_write
- Source: https://github.com/nothings/stb
- License: Public domain or MIT (see `CrescentEngine/Engine/Rendering/stb_image_write.h`)
